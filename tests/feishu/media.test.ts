...
import { FeishuMediaService } from '../../src/feishu/media.js'
import { AttachmentStore } from '../../src/feishu/attachment/attachment-store.js'
...
describe('FeishuMediaService', () => {
  ...
  // ── 回归：node-sdk 实际返回已解包形态（resp.file_key / resp.image_key 在顶层）──
  // 2026-08-18 真机实测发现：im.file.create 返回 { file_key: "file_v3_..." }（lib/index.js
  // return res?.data || null），旧代码取 resp?.data?.file_key 永远 undefined → missing file_key。
  it('uploadImage accepts top-level image_key (real SDK unwrapped shape)', async () => {
    const client = makeMockClient()
    ;(client.im.image.create as any).mockResolvedValue({ image_key: 'img_top_789' })
    const store = new AttachmentStore({ root: tmpRoot, retentionMs: 60_000 })
    const svc = new FeishuMediaService(client as any, store)
    await expect(svc.uploadImage(Buffer.from('PNGDATA'), 'image/png')).resolves.toBe('img_top_789')
  })

  it('uploadFile accepts top-level file_key (real SDK unwrapped shape)', async () => {
    const client = makeMockClient()
    ;(client.im.file.create as any).mockResolvedValue({ file_key: 'file_v3_top_789' })
    const store = new AttachmentStore({ root: tmpRoot, retentionMs: 60_000 })
    const svc = new FeishuMediaService(client as any, store)
    await expect(svc.uploadFile(Buffer.from('PDFDATA'), 'report.pdf')).resolves.toBe('file_v3_top_789')
  })

  it('uploadImage still works with wrapped { data: { image_key } } shape', async () => {
    const client = makeMockClient()
    const store = new AttachmentStore({ root: tmpRoot, retentionMs: 60_000 })
    const svc = new FeishuMediaService(client as any, store)
    await expect(svc.uploadImage(Buffer.from('x'), 'image/png')).resolves.toBe('img_fake_123')
  })

  it('uploadFile still works with wrapped { data: { file_key } } shape', async () => {
    const client = makeMockClient()
    const store = new AttachmentStore({ root: tmpRoot, retentionMs: 60_000 })
    const svc = new FeishuMediaService(client as any, store)
    await expect(svc.uploadFile(Buffer.from('x'), 'a.txt')).resolves.toBe('file_fake_456')
  })

  it('uploadImage throws when neither shape carries a key', async () => {
    const client = makeMockClient()
    ;(client.im.image.create as any).mockResolvedValue({})
    const store = new AttachmentStore({ root: tmpRoot, retentionMs: 60_000 })
    const svc = new FeishuMediaService(client as any, store)
    await expect(svc.uploadImage(Buffer.from('x'), 'image/png')).rejects.toThrow('missing image_key')
  })

  it('uploadFile throws when neither shape carries a key', async () => {
    const client = makeMockClient()
    ;(client.im.file.create as any).mockResolvedValue({ data: {} })
    const store = new AttachmentStore({ root: tmpRoot, retentionMs: 60_000 })
    const svc = new FeishuMediaService(client as any, store)
    await expect(svc.uploadFile(Buffer.from('x'), 'a.txt')).rejects.toThrow('missing file_key')
  })
})
